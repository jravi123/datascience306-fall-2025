...
```{r}
my_date <- as_datetime("2017-01-02 20:11:59")
print(my_date)
...
But a better function for String represented dates is `parse_date_time`:
...
Another example
```{r}
parse_date_time("2023 Jan 01, 12:00:00", "Y b d, H:M:S")
```
...
```{r}
right_now <- now()
day(right_now) # has an alias - mday()
...
# setting reminders
## method 1
ds_parsed2 = ds_parsed - months(1)
year(ds_parsed2) = year(now())
if_else(ds_parsed2 < now(), ds_parsed2 + years(1), ds_parsed2)
...
In this lecture, we covered the basics of working with dates in R.

...
If two events are independent, the probability of both happening is the product of their individual probabilities:
...
```{r lec12-02-solution}
"SELECT manufacturer, MAX(hwy) FROM mpg GROUP BY manufacturer"
```

## Exercise
How many 4-cylinder models are present in this database?

```{r lec12-03, exercise = TRUE}
con <- dbConnect(RSQLite::SQLite(), ":memory:")
dbWriteTable(con, "mpg", mpg, overwrite = T)
# replace ... with your query
res <- dbSendQuery(con, "....")
...
```{r lec12-03-solution}
"SELECT COUNT(*) FROM mpg WHERE cyl = 4"
```
...
```{r ex-5-solution}
"SELECT manufacturer, model, MIN(hwy) FROM mpg GROUP BY manufacturer, model"
```
...
```{r ex-6-solution}
"SELECT manufacturer, class, COUNT(*) FROM mpg WHERE class = 'compact' GROUP BY manufacturer HAVING COUNT(*) >= 4"
```

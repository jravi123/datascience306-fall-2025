...
* `group_by` takes one or more grouping factors - useful for finding conditional summaries, transformations, or filters within subsets of data. `mpg |> group_by(manufacturer) |> summarize(hwy_mean = mean(hwy))`

## Today we will learn 

`more groups and summaries`
...
`nycflights13` is a popular R package that contains a collection of five related datasets about flights that departed from New York City in 2013. 

Using the below command, you can get a handle on these datasets

`library(nycflights13)`

One of the datasets is `flights`. Let's take a glimpse on `flights` dataset
...
In the flights data, let us find the mean departure delays for each carrier.

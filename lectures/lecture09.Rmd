...
# another way
flights |> reframe(carrier = unique(carrier)) |>
    left_join(airlines, join_by(carrier))
```
...
The table that has a foreign key column will have duplicate values in a one-to-many relationship.

This relationship is useful when you want to add additional information for every primary key.
...
When we do a join, R uses any columns with matching names from both data frames as the keys to merge them:
...
Frequency distribution of the top 6 destinations can also be calculated as:

```{r}
flights |> count(dest) |> slice_max(n, n = 6) |> semi_join(x = flights) |> ggplot(aes(x = dest, fill = dest)) + geom_bar() 
```

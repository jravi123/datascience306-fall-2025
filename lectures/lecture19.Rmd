...
*Residual Standard error* is the total variation in the residual values.

If the residuals are distributed normally, the first and third quantiles of the
residuals should be about 1.5 times this standard error.
...
int00.dat |> add_residuals(int00.lm) |> add_predictions(int00.lm) |>
    ggplot() + geom_point(aes(x = resid, y = pred, color = clock))
```

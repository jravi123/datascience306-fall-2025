---
subtitle: "DATASCI 306: Lecture 19"
...
The process of *fitting* a model refers to selecting a particular choice 
...
$y = a_0 + a_1x$
If we select a particular `$a_0 and `$a_1, this gives us a potential model for the data. We can plot this for various choices of `$a_0 and `$a_1 and visually see how well it might fit:
...
The value `$\hat{y}_i - y_i is called the *residual*. It's the component of the data that isn't predicted by our model. Adding up the residuals gives us a measure of how well our model fits the data. 
...
To fit a linear model in R, we use a special formula syntax which is denoted by `~`:

`lm(y ~ x, data)`

This says to regress `data$y` on `data$x`.
...
It means that over all possible choices of `$a_0 and `$a_1, the ones selected by `lm()`, which we call `$\hat{a}_0 and `$\hat{a}_1, minimize the squared error (using OLS - Ordinary Least Squares):
...
## Exercise
According to this model, what is the predicted value of `y` when `x = 0`?
...
* The Student's `$t-value is nothing but the estimate / `std.error`.
...
If the linear model is correct, we would expect residual values that are normally distributed around a mean of zero, a median value near zero, minimum and maximum values of roughly the same magnitude, and first and third quartile values of roughly the same magnitude. 
...
The last few lines of the summary output contain some information about the overall fit of the linear model to this dataset:
...
* `$F-statistic: a test of the hypothesis that all of the coefficients in the model are simultaneously zero.
...
The first dataset that we will use to practice regression comes from the e-book given for reading. It consists of data on computer processors and their performance-related metrics.
...
We will use `int00.dat` in this lesson. Now, let's take a look at the first few records of this dataframe.
...
`perf` measures performance, and `clock` measures clock speed. Are they linearly related?
...
In the next lecture, we'll talk about multiple linear regression, which lets us take into account multiple predictors at a time.

...
library(tidyverse)
library(modelr)
processors <- read.csv("data/all-data.csv")
...
* How to interpret the multiple linear model and `summary.lm()`.
...
In the last lecture, we saw the `sim1` data that had:
...
When `x=a`, each of `{x=b}`, `{x=c}`, and `{x=d}` are 0. Therefore, the regression model for the subset of observations that have `x=a` is just:
...
Multiple linear regression is a generalization of the simple linear regression model discussed in the last lecture. Now we assume that each observation $y$ is a linear combination of several predictors $x_1, \dots, x_k$:
...
## Interpreting the multiple linear model
...
Let's add the predicted values and then plot them:
...
* Smaller models are more interpretable. If your goal is to understand the process(es) that generated your data, then you should try to find the smallest possible model that has good explanatory power.
...
Here is the potential list of predictors we can start with:
...
Let's try the following strategy to remove some of the variables while still retaining a model that has good explanatory power:
...
Let's now update our model by removing this predictor using the `update` function.

```{r}
int00.lm.2 <- update(int00.lm.full, .~. - FO4delay, data = int00.dat)
summary(int00.lm.2)
```
Similarly, drop `featureSize`, `transistors`, and `dieSize`.

Along the way, notice that:

* the missingness value improves as fewer and fewer observations are dropped along the way (the features contributing to missingness are eliminated one by one).
* the degrees of freedom increase accordingly as well.



## What could go wrong?

Let's look at another dataset, `int92.dat`. This contains the data from the CPU DB database for all of the processors for which performance results were available for the SPEC Integer 1992 (Int1992) benchmark program.
...
Hmm... let's figure out how many observations we have.

```{r}
nrow(int92.dat)
```

Let's take a look at the `clock` values.

```{r}
int92.dat |> count(clock)
```

And now the `threads` column values.
...
## Exercise
Is considering `cores` as a predictor a good idea?
...
In regression, we have a continuous outcome (height, population growth, etc.) and some predictors that could be either categorical or continuous. We built a function that predicts the mean of the outcome given the predictors, e.g.,
...
we are going to look at a different kind of regression model. The main difference is that our outcome is no longer a continuous variable like price or `log(price)`. Instead, the outcome is a true/false variable that indicates whether or not an event happened.

## Deaths on the RMS Titanic
The [RMS](https://en.wikipedia.org/wiki/Titanic) Titanic famously sank on April 15, 1912, during its maiden voyage, killing about two-thirds of its passengers. The `titanic` dataset records the fates of everyone aboard.
...
titanic_df |> head()
```
The last `n` column provides the total number of records with the same values in the other columns. So, let's replicate the rows to match the `n` value as shown below:
...
ggplot(titanic_df) + geom_bar(
  aes(x = interaction(Class, Sex, Age), 
      fill = Survived), 
  position = 'dodge') + coord_flip()
```

We have three categorical predictors: `Sex`, `Age` (adult/child), and `Class` (1st, 2nd, 3rd, or Crew). Our outcome is also categorical: survived or not.

## Logistic regression

Suppose we want to build a regression model for predicting categorical outcomes. In all of the regressions we have seen so far, the dependent ($y$) variable has been continuous: population growth, life expectancy, price, etc. However, there are many situations where the outcome is categorical. For simplicity, we will assume it's binary and can be coded as 0/1. How should we model binary data?

One idea is to just stick with what we know and use the linear model. Will this work?

```{r}

mdl <- lm(Survived ~ Class + Sex + Age, 
          data = titanic_df)
summary(mdl)
```
...
For $f(x)=x$, and for 0/1 outcomes, let's instead try to think probabilistically:
...
* For "extreme" values of $a + bx$, it should tend to zero or one.
 ...
 A function that satisfies both of these properties is the `logistic function`:

...
Note: Note how the lengths of the columns is made the same for data.frame by repeating the elements.

## What We Know

* Pulling out a single column: `d$col` 
    alternatively you can use `pull(d, col)`
...
```{r}
select(storms, name, status) |> head()
```

## Getting all but some columns

```{r}
select(mpg, !drv) |> head(4)
```

Alternatively
```{r}
select(mpg, -drv) |> head(2)
```
...
```{r}
select(storms, where(is.character)) |> head()
```
...
We see a right-skewed (or positively skewed) distribution when we compute the `z` for temp difference
...
## A Two-Step Approach to solutions

Sometimes, a data analysis question is too complex to solve in a single step. A powerful strategy is to break the problem down into smaller, manageable pieces. Often, this involves a two-step process:

1.  **Calculate an intermediate result:** Perform some calculations (e.g., group and summarize) to find a specific subset of your data you're interested in (like the "top 5" or "worst 3").

2.  **Use the intermediate result to filter and visualize:** Use the result from the first step to filter your original dataset and then create a plot or perform further analysis on that subset.

## Group records using `group_by`
...
## Example: Top Manufacturers by Fuel Efficiency

Let's find the 5 manufacturers with the highest average highway mileage (`hwy`) and then create a boxplot of their city mileage (`cty`).

**Step 1: Find the top 5 manufacturers**

First, we'll calculate the average `hwy` for each manufacturer and select the top 5. We'll store their names in a vector.

```{r}
top_5_manufacturers <- mpg |>
...
**Step 2: Filter the original data and plot**

Now, we use the `top_5_manufacturers` vector to filter the original `mpg` dataset and create a boxplot of their city mileage.

```{r}
mpg |>
  filter(manufacturer %in% top_5_manufacturers) |>
  ggplot(aes(x = cty, y = manufacturer)) +
  geom_boxplot() +
  labs(title = "City Mileage for Top 5 Manufacturers by Highway Mileage",
       x = "City Mileage (cty)",
       y = "Manufacturer")
```

This two-step approach makes the code easier to read, write, and debug.

...
Faceting allows us to split data into subsets and plot similar graphs for each subset. 
In lecture we saw that facets are often very useful to avoid overplotting. 
...
Now if we want to summarize the distribution for tooth growth for each supplement, we can facet on supplement: 
...
This is a famous statistical dataset that has been cited in many papers.
...
**Exercise**: Add facets and colors to the following code to reduce over-plotting. 
...
In this plot, he points out an oddity in Morris: it's the only site in which the 1932 crop yield was higher than that in 1931! 
...
> - `coord_flip()`: Default coordinate system, but flipping the `x` and `y` axes. 
> - `coord_fixed()`: Default coordinate system, but fixing the ratio between the `x` and `y` axes. 
...
The idea for polar coordinate mapping is that sometimes displaying information as an angle and radius is more informative than displaying it in the `x, y` Cartesian plane.
...
Let's save the map to use later:
...
The problem with the previous slide is that the polygons are a bit distorted, so they don't look exactly as we would expect.

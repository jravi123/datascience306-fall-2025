...
Git is a very important version control system that everyone should get familiar with. It may possibly come handy in your future too! While it may be intimidating at first, your effort will be well worth it in the long run.
Git was already discussed in some detail in the first lecture, and now we will work on some basic git commands that you all should know.
...
* Put `https://github.com/jravi123/datascience306-fall-2025` as the value for the text input box for `URL of your New Repository`. 
...
`help(sqrt)` also works - similar to `?sqrt`
...
Subsetting is used to select a part of an object (like a vector or a list). R has a number of operators that can be used for this purpose.

- `[ ]` is the basic subsetting operator. It can be used to select one or more elements from a vector or a list. When used on a list, it will return a new, smaller list.
- `[[ ]]` is used to extract a single element from a list or a vector. It is often used with lists to get the object inside a list element.
- `$` is a convenient shorthand for `[[ ]]` to extract a single element by name from a list or data frame.

We can subset vectors in several ways: by position, by name, or using a logical vector.

**Subsetting by position:**

You can select elements by their index (position).
...
We can also use a logical vector to select elements. The logical vector should have the same length as the vector you are subsetting. `TRUE` means keep the element, `FALSE` means drop it.

```{r logical_subset}
fp_vec <- c(-0.3, 16.7, 58.9) # from before
fp_vec > 10 # this creates a logical vector
fp_vec[fp_vec > 10] # use it to subset
```

**Subsetting named vectors:**

If a vector has names, you can use them to select elements.

```{r named_subset}
named_string_vec
named_string_vec["fruit2"]
named_string_vec[c("fruit1", "fruit3")]
```

**Subsetting lists:**

Subsetting lists is a bit different. Using `[ ]` on a list will always return a list. If you want to get the object *inside* a list element, you need to use `[[ ]]` or `$`.

```{r list_subset}
my_list[1] # This is a list containing the first element of my_list


my_list[[1]] # This is the first element of my_list itself


# Using names
named_list$the_int_vec # Extract component named "the_int_vec"
named_list[["the_int_vec"]] # Same as above
named_list["the_int_vec"] # This returns a list!
```
